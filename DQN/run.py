...
# 加载 YOLOv5 模型
model_path = r'..\lighting\yolov5\runs\train\yolov5_lighting10\weights\best.pt'
...
def letterbox(img, new_shape=(1920, 1920), color=(114, 114, 114), auto=True, scaleFill=False, stride=32):
    ...
    # if True:
    #     cv2.imshow('Letterboxed Image', new_image)
    #     cv2.waitKey(0)  # 等待按键
...
if __name__ == "__main__":
    # 提取图片特征向量
    image_path = r'..\lighting\image\frame_0007.jpg'  # 替换为你的图片路径
    feature_vector = extract_features(image_path, model, img_size=(1920, 1920))

    # 打印特征向量的形状
    print("特征向量形状:", feature_vector.shape)

    # 将特征向量转换为numpy数组
    feature_vector_numpy = feature_vector.cpu().numpy()

    # 将特征向量作为输入放入DQN模型中
    # 假设你有一个DQN模型实例化为 dqn_model
    # 例如：dqn_model.forward(feature_vector_numpy)  # 根据需要进行模型前向传播
